            ...
            <div class="container">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered">
                        ...
                        <%
                        	ResultSet totalProduct = DatabaseConnection.getResultFromSqlQuery("select tblproduct.image_name, tblproduct.name,tblproduct.productQuantity, tblcart.quantity as quantity, tblcart.mrp_price, tblcart.discount_price, tblcart.total_price, tblcart.product_id from tblproduct, tblcart where tblproduct.id = tblcart.product_id and customer_id='"
                        ...
                        <tr class="rem1">
                            ...
                            <td class="invert"><img
                                    ...
                                    class="pro-image-front" style="width: 150px; height: 100px;"><br><%=totalProduct.getString(2)%>
                             </td>
                             <td class="invert">
        						<!-- Display product quantity in stock -->
        						<%=totalProduct.getInt("productQuantity")%>
    						</td>
                            <td class="invert">
                                <div class="quantity">
                                    <div class="quantity-select">
                                        <form action="UpdateProductQuantity" method="post">
                                            <input type="hidden" value="<%=totalProduct.getInt(8)%>"
                                                   name="productId"> <input type="number"
                                                   name="quantity" value="<%=totalProduct.getInt(4)%>"
                                                   style="width: 50px; height: 35px;">&nbsp;<input
                                                   type="submit" class="btn btn-danger" value="Change" >
                                        </form>
                                        <%
						                    // Check if the requested quantity is greater than the quantity in stock
						                    if (totalProduct.getInt(4) > totalProduct.getInt("productQuantity")) {
						                %>
						                <p style="color: red;">Not enough stock available! Original quantity: <%=totalProduct.getInt("productQuantity")%></p>
						                <%
						                    }
						                %>
                                    </div>
                            ...
                            <td class="invert"><del><%=totalProduct.getString(5)%>&nbsp;ETB.
                                </del></td>
                            <td class="invert"><%=totalProduct.getString(6)%>&nbsp;ETB.</td>
                            <td class="invert"><%=totalProduct.getString(7)%>&nbsp;ETB.</td>
                            <td class="invert"><a
                                    href="removeProductFromCart.jsp?productId=<%=totalProduct.getInt(8)%>"
